...
# scripts, mise config, pyproject.toml, or the test files and your push
...
# (bloom/__init__.py) and any non-canonical edits are left alone. The
# script aborts if you have local uncommitted changes to any canonical
...
#   TEMPLATE_OVERRIDE=acme/byox-bloom-filter-python bash .karnstack/sync.sh

set -euo pipefail

TEMPLATE="${TEMPLATE_OVERRIDE:-karnstack/byox-bloom-filter-python}"
